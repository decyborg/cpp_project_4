...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    // Check if the light is currently red, if so wait for it to turn green, otherwise it is already green and we can proceed
    auto curr_state = _trafficLight.getCurrentPhase();
    lck.lock();
    std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " is light " << static_cast<bool>(curr_state) << std::endl;
    lck.unlock();
    if(curr_state == TrafficLightPhase::red)
    {
